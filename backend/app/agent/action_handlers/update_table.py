...
class UpdateTableHandler:
    ...
    def _resolve_params(self, params: dict, context: ExecutionContext) -> dict:
        ...
        rows_ref = resolved.pop("rows_from_artifact", None)
        if rows_ref:
            artifact = context.require_artifact(rows_ref)
            resolved["rows_payload"] = artifact.value
